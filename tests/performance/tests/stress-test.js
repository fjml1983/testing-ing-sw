...
export const options = {
  stages: [
    ...
    { duration: '2m', target: 1500 },  // Carga constante: 20 usuarios (punto crítico para SQLite)
    //{ duration: '1m', target: 50 },  // Stress: subir a 50 para forzar el bloqueo
    { duration: '1m', target: 0 },   // Rampa de bajada
  ],
  thresholds: {
    http_req_failed: ['rate<0.1'], // Falla si más del 5% de los inserts dan error
    http_req_duration: ['p(95)<2000'], // El 95% de los inserts deben ser < 1s
  },
};
...
export default function () {
  ...
  const payload = JSON.stringify({
    title: `Rocky ${Math.floor(Math.random() * 1000000)}`,
    year: '2025',
    format: 'DVD',
    operation: 'venta',
    memberName: 'OSWALDO GUZMAN PLAZA',
  });
